...
df_barcode1 <- read_csv("Barcode 1 Taxonomy_RA Sorted_1_.csv")
df_barcode2 <- read_csv("Barcode 2 Taxonomy_RA Sorted_1_.csv")
...
df_barcode1_tocombine <- df_barcode1_tosort %>%
  mutate(Sample = "Healthy1_Run1")
df_barcode2_tocombine <- df_barcode2_tosort %>%
  mutate(Sample = "Diseased1_Run1")
...
df_combined <- bind_rows(df_barcode1_tocombine,
                         df_barcode2_tocombine,
                         df_barcode11_tocombine,
...
#Step 2: filtering the TOP 20 - we make a reference sheet to then compare to the original datafram
df_top10_list <- df_RAtotaled %>% 
  slice_max(order_by = Total_RA, n = 10)
df_top10_list

#reintroducing this Healthy and Diseased labels into the table
#BY combining the df_combined (original graph) to the df_top10 (which
...
stacked_plot_combined <- df_combined_top10 %>%
  ...
  #labelling the plot
  labs (title = "Top 10 Species for Healthy vs. Diseased across both runs",
  ...
  scale_fill_manual(
    values = c(
      "Diseased1_Run1" = "red",
      "Diseased1_Run2" = "firebrick",
      "Diseased2_Run2" = "indianred",
      "Healthy1_Run1"  = "green",
...
write_csv(df_combined_top10, "Healthy vs Diseased_ALL FECAL RUNS_without Zymo.csv")
